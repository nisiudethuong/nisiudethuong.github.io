...
// Guards để tránh attach listener nhiều lần (nguyên nhân gây nhân đôi khi tạo diary/photo/message)
let baseWired = false;
let privateWired = false;
...
function lockApp() {
  ...
  // Cho phép wire lại listener khi unlock lần sau (vì DOM mới từ template)
  privateWired = false;
}
...
function unlockApp() {
  ...
  // Wire listeners cho private UI (chỉ 1 lần cho đến khi lock lại)
  wireEvents();
}
...
function ensureUnlockedUI() {
  ...
  if (ok) {
    ...
    // Wire private listeners một lần
    wireEvents();
  } else {
    lockApp();
  }
...
function wireEvents() {
  // --- BASE (lock screen) - chỉ attach 1 lần duy nhất ---
  if (!baseWired) {
    els.lockForm.addEventListener("submit", async (e) => {
      ...
      if (v === APP_PASSCODES.unlock) {
        setStatus(els.lockError, "");
        unlockApp();   // bên trong unlockApp đã gọi wireEvents() + mount
        refreshAll();
      } else {
    ...
    // Keydown cho photo viewer (chỉ cần attach 1 lần)
  ...
  // --- PRIVATE UI (tabs, forms, lists...) ---
  // Chỉ attach khi private UI đã mount và chưa wire lần này
  if (privateWired) return;
  // Chỉ wire private khi đã có ít nhất 1 element đặc trưng (ví dụ diaryForm hoặc tabs)
  if (!els.diaryForm && !els.tabs.length) return;
  ...
  // Confirm dialog wiring (các dialog này nằm trong template)
...
function boot() {
  sb = initSupabase();
  ensureUnlockedUI();
  wireEvents();           // base listeners (lockForm, btnLock, keydown). Private listeners sẽ attach sau khi mount UI từ template.
  switchTab("diary");
  ...
  if (localStorage.getItem("love_unlocked") === "1") {
    // private wiring đã được gọi bên trong ensureUnlockedUI (sau mount)
    refreshAll();
  }
}
